...
redirect('login');
	...
	public function editaddress()
	{
		...
		$id=$this->uri->segment(2);
		 ...
		 $num_check=$app->checkeditaddress($id,$userid); 
...
public function updateaddress(){
	...
	public function order_details()
	{
		...
		$data['ordernotes']=$app->ordernotes($orderid);
       ...
       //print_r($data['filearr'][$itemid]);
		...
		$data['shipcountryarr']=$app->getcountryname($scode);
           ...
           $this->db->update('order_comments', $data_comment);
				...
				foreach ($data[order_details] as $items)
{
		...
		public function notifications()
	{
		...
		$data['notifications'] = $app->getnotifications($userid);  
		...
		$this->template->load('frontend/myaccount', 'notifications', $data);
