...
class Supplier extends MY_Controller
{
        ...
        if($this -> ion_auth -> logged_in()==1)
        {
           ...
           // $data['products'] = $review->productslisting();
            ...
            else
            {
				...
				$supplier_arbic               = $this->input->post('supplier_arabic');
				...
				$status                = $this->input->post('status');
                 ...
                 #supplier INSERT
                        $data               = array(
							...
							'supplier_name_arabic'                   =>  $supplier_arbic,                            
                            'supplier_description_arabic'            =>  $description_arabic ,
							 'contact_email'                   =>  $email, 
							 'contact_phone'                   =>  $phone, 
					   ...
					   {
                        $complted= $this->db->insert('suppliers', $data);
					   }
	...
	public function edit()
		...
		$access=$pageacces->getpageaccess($adminid);
		  ...
		  $data['result']=$supplier->editsupplier($supplier_id);
		  $this->template->load('admin_blank','add_supplier',$data);
    ...
    function supplier_list(){
        ...
        if($this->ion_auth->logged_in()==1)
        {
			$supplier = new App();
            $this->app = new App();
			$data['suppliers'] = $supplier->supplierlisting();
			//print_r($data['categories']);
            
            $this->template->load('admin_blank', 'supplier_list',$data);
        }
        else
        {
