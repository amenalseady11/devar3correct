...
<div  id="banner" class="innerBanner productBanner">
			...
			<div class="row">
				...
				<div class="col-md-8 col-lg-9">					
				   ...
				   <?php
						...
						$namearr[]=str_replace(" ","",strtolower($val->name));
	...
	$i="0";
	                     ...
	                     {
											$vname=str_replace(" ","",strtolower($v->name)); 
											 
										if(in_array($vname,$namearr))	 {
...
<span id="<?php echo $attvalue?>"><?php echo $options[$i-1]->value;?></span>
					   ...
					   {;
									...
									if($b==$options[$i-1]->value)	{ $sel="checked";} else {$sel="";}
										?>
										<div class="price-calculator-items">	
											<label data-toggle="collapse" data-parent="#accordion" href="#collapse<?php echo $as;?>">
												<input type="radio" name="<?php echo $v->name?>" value="<?php echo $b?>" class="ab" <?php echo $sel;?>>
					...
					$ab=$i;
					 ...
					 $collapse="collapse";
									...
									//echo $ab."<br>";
											...
											//if(count($data[$attvalue]) >="1"){
											 
				
                            
					   ?>
							
							<div class="card" id="editnoncid_<?php echo $m?>">
								<div class="card-header" id="heading<?php echo $ab?>" data-toggle="collapse" data-target="#collapse<?php echo $ab?>" aria-expanded="<?php echo $exp ?>" aria-controls="collapse<?php echo $ab?>">
									<h5 class="mb-0"><?php echo $v->name?></h5>
<span id="<?php echo $attvalue?>"><?php echo $options[$i-1]->value;?></span>
								</div>
		...
		<!-- Dropzone -->
	   	<form action="<?= base_url('home/fileupload') ?>" class="dropzone" id='fileupload'>&nbsp;
			...
			<div class="detailsPriceBody">
							<p>
								<span>Quantity </span>
								<input class="form-control" type="text" name="quantity" id="quantityid" value="<?php echo $qty?>" >
								<input type="hidden" name="perprice" id="perpriceid" value="<?php echo $perprice?>">
								<input type="hidden" name="orgprice" id="orgpriceid" value="<?php //echo $pr['price']?>">
								<input type="hidden" name="min_quantity" id="min_quantityid" value="<?php echo $qty?>">
