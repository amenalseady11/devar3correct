						...
						<div>
							...
							<nav aria-label="breadcrumb">
								...
								<li class="breadcrumb-item active">Category</li>
									<li class="breadcrumb-item active" aria-current="page">New Productmap</li>
								</ol>
							</nav>
						</div>
...
<?php
    ...
    foreach ($products as $prd) {?>
								...
								<option value="<?php echo $prd->product_id?>" <?php if(isset($result->category))if($result->category==$prd->product_id) echo "selected";?>><?php echo $prd->product_name?></option>					
    ...
    foreach ($dependattributes as $dat) {?>
						...
						<option value="<?php echo $dat->dependable_attribute_id?>|<?php echo $dat->dependable_attribute_name?>" <?php if(isset($result->category))if($result->category==$cat->dependable_attribute_id) echo "selected";?>><?php echo $dat->dependable_attribute_name?></option>					
   ...
   <div class="row">
    ...
    foreach ($nondependattributes as $dat) {?>
								...
								<option value="<?php echo $dat->dependable_attribute_id?>|<?php echo $dat->dependable_attribute_name?>" <?php if(isset($result->category))if($result->category==$dat->non_dependable_attribute_id) echo "selected";?>><?php echo $dat->dependable_attribute_name?></option>					
            ...
            <!--   <button type="button" id="remove-button1" class="btn btn-danger float-left  ml-1" disabled="disabled"><i class="fas fa-minus fa-fw"></i> Remove</button> -->
                        ...
                        <option value="2" <?php if(isset($result->status))if($result->status=="2") echo "selected";?>>InActive</option>
										...
										<div class="col-12 col-lg-6 text-right">
											<br>
											<button class="btn btn-main-primary pd-x-20 mg-t-10" type="submit" id="submit">Submit Form</button> <a href="<?php echo base_url()?>productsmap/productmap_list" role="button" class="btn btn-dark pd-x-20 mg-t-10">Cancel</a>                   </div>
							                </div>
								</form>
